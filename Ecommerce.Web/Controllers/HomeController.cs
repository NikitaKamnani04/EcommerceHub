...
namespace Ecommerce.Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? search = null, int page = 1)
        {
            ...
            // Query string banate hain - search aur page dono optional
            var queryString = $"api/Products?page={page}&pageSize=12";
            if(!string.IsNullOrWhiteSpace(search))
            {
                // Uri.EscapeDataString - special characters (spaces, &, etc.) ko URL-safe banata hai
                queryString += $"&search={Uri.EscapeDataString(search)}";
            }

            var pagedResult = await _apiService.GetAsync<PagedProductsViewModel>(queryString);
            pagedResult ??= new PagedProductsViewModel();

            pagedResult.SearchTerm = search; // View mein search box pre-fill karne ke liye
